...
def build_interaction_matrix(corners, Z):
    rows = [interaction_row(u, v, Z) for u, v in corners]
...
class IBVSController:
    def __init__(self, target_corners):
        self.target_corners_px = np.asarray(target_corners, dtype=float).ravel()
        self.s_star = to_normalized(target_corners).ravel()
    ...
    def feature_error_norm(self, corners):
        ...
        return np.linalg.norm(corners - self.target_corners_px)
