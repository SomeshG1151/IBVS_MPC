...
TARGET_CORNERS = make_target_corners(
    FRAME_W,
    FRAME_H,
    half_side=80,
)
...
def send_attitude(conn, roll, pitch, thrust):
    roll = np.clip(roll, -0.40, 0.40)
    pitch = np.clip(pitch, -0.35, 0.35)
    ...
    try:
        while True:
            ...
            if not valid or tracker.lost > LOST_LIMIT:
                ...
                vc, e, L = ibvs.compute(corners, Z=ASSUMED_DEPTH)

                vc_clipped = np.clip(vc, -VC_CLIP, VC_CLIP)
                x_star = mpc.compute_reference(vc_clipped, yaw=x_now[5])

                # rate-limited MPC solve: only solve at MPC_RATE Hz
                now = time.time()
                ...
                cv2.putText(frame,
                            f"cmd=[{np.degrees(roll_cmd):.1f},{np.degrees(pitch_cmd):.1f}]  nuz={nu_z:.1f}N",
