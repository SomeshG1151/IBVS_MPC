...
class LinearMPC:
    def __init__(self):
        ...
        self.X_STAR_ALPHA = 0.2

        self.Fx, self.Fu = self._build_condensed_prediction()

        self.Q_bar = block_diag(
            *([self.Qx] * N + [self.Qf])
        )

        self.R_bar = block_diag(
            *([self.Qu] * N)
        )

        H_mat = (
            self.Fu.T @ self.Q_bar @ self.Fu
            + self.R_bar
        )

        H_mat = 0.5 * (H_mat + H_mat.T)

        self.RATE_WEIGHT = 5.0
        self.H_mat = (
            H_mat
            + self.RATE_WEIGHT * np.eye(H_mat.shape[0])
        )
        ...
        print(
            f"[MPC] ROLL_MAX={ROLL_MAX:.3f} rad  "
            f"PITCH_MAX={PITCH_MAX:.3f} rad  "
            f"NU_Z_HOV={NU_Z_HOV:.2f} N"
        )

        print(
            f"[MPC] A norm={np.linalg.norm(self.A):.3f}  "
            f"B norm={np.linalg.norm(self.B):.3f}"
        )

        print(
            f"[MPC] TAU_RP={TAU_RP}s  "
            f"X_STAR_ALPHA={self.X_STAR_ALPHA}"
        )

    def compute_reference(self, vc, yaw=0.0):
        R_Bc = np.array([
            [0, 0, 1],
            [1, 0, 0],
            [0, 1, 0],
        ...
        self._x_star_filt = (
            self.X_STAR_ALPHA * x_star
            + (1.0 - self.X_STAR_ALPHA)
            * self._x_star_filt
        )
